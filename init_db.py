...
q2 = b"CREATE USER '" + bytes(conf['username'], encoding='ascii') + b"'@'%' IDENTIFIED BY '" + bytes(conf['password'], encoding='ascii') + b"';\n"
...
q4 = b"GRANT SELECT, INSERT ON `PlayerCount` TO '" + bytes(conf['username'], encoding='ascii') + b"'@'%';\n"
